...
from menu.markups import confirm_markup, startup_markup, cancel_markup
from menu.replies import load_content_msg, load_style_msg, confirm_msg, run_msg, nst_end_msg, nst_cancel_msg
import asyncio
import os
...
async def run_style_transfer(message: types.Message):
    ...
    await message.answer(text=load_content_msg, reply_markup=cancel_markup)
...
async def load_content(message: types.Message, state: FSMContext):
    ...
    await message.answer(text=load_style_msg, reply_markup=cancel_markup)
...
async def load_style(message: types.Message, state: FSMContext):
    ...
    await FSMStyleTransfering.next()
    await message.answer(text=confirm_msg, reply_markup=confirm_markup)
...
async def confirm(message: types.Message, state: FSMContext):
    ...
    await message.answer(text=run_msg, reply_markup=ReplyKeyboardRemove())
    ...
    await message.answer(text=nst_end_msg, reply_markup=startup_markup)
...
async def cancel_style_transfer(message: types.Message, state: FSMContext):
    ...
    await clear_files(state)
    await state.finish()
    await message.answer(text=nst_cancel_msg, reply_markup=startup_markup)
